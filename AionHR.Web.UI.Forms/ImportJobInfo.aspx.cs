...
{
    public partial class ImportJobInfo : System.Web.UI.Page
    {
        ...
        protected void ProcessData(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                JobInfoBatchRunner runner = new JobInfoBatchRunner(storage, _system,cs, emp) { Items = shifts, OutputPath = MapPath("~/Imports/" + _systemService.SessionHelper.Get("AccountId") + "/") };
                runner.Process();
        ...
        protected void DownloadResult(object sender, DirectEventArgs e)
        {
            ...
            try
            {
                content = File.ReadAllText(MapPath("~/Imports/" + _systemService.SessionHelper.Get("AccountId") + "/" + ClassId.EPJI.ToString() + ".csv"));
            }
