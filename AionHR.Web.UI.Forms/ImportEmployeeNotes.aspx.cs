...
{
    public partial class ImportEmployeeNotes : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!X.IsAjaxRequest && !IsPostBack)
            {
                ...
                try
                {
                    AccessControlApplier.ApplyAccessControlOnPage(typeof(AionHR.Model.Employees.Profile.ImportNotes), null, null, null, null);
                }
        ...
        protected void ProcessData(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                EmployeeNotesImportingService service = null;
                service = new EmployeeNotesImportingService(new CSVImporter(CurrentPath.Text));
                List<EmployeeNote> shifts = service.ImportUnvalidated(CurrentPath.Text);
                ...
                EmployeeService _timeAtt = new EmployeeService( new EmployeeRepository(),h);
                SystemService _system = new SystemService(new SystemRepository(), h);
                EmployeeNotesBatchRunner runner = new EmployeeNotesBatchRunner(storage, _timeAtt, _system,  emp) { Items = shifts, OutputPath = MapPath("~/Imports/" + _systemService.SessionHelper.Get("AccountId") + "/") };
                runner.Process();
        ...
        protected void DownloadResult(object sender, DirectEventArgs e)
        {
            ...
            try
            {
                content = File.ReadAllText(MapPath("~/Imports/" + _systemService.SessionHelper.Get("AccountId") + "/" + ClassId.EPNO.ToString() + ".csv"));
            }
