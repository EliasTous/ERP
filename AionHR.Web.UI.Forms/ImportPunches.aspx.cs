    ...
    public partial class ImportPunches : System.Web.UI.Page
    {
        ...
        protected void ProcessData(object sender, DirectEventArgs e)
        {
            ...
            catch (Exception exp)
            {
               ...
               for(int i=0; i< errorDetails.Length-2;i++)
                {
                    errorDetail += errorDetails[i] + " ";
                }
               
                  if (errorDetails.Length >0)
                    X.Msg.Alert(Resources.Common.Error, GetGlobalResourceObject("Common", "FieldDetails") + ":" + errorDetail + "<br />" + GetGlobalResourceObject("Common", "ExceptionMessage") + exp.Message).Show();
