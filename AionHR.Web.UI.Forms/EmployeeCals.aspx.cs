...
{
    public partial class EmployeeCals : System.Web.UI.Page
    {
        ...
        protected void PoPuPEH(object sender, DirectEventArgs e)
        {
            ...
            switch (type)
            {
                case "imgEdit":
                    ...
                    RecordResponse<EmployeeCal> response = _employeeService.ChildGetRecord<EmployeeCal>(r);
                    if (!response.Success)
                    {
                        X.MessageBox.ButtonText.Ok = Resources.Common.Ok;
                ...
                case "imgDelete":
                    X.Msg.Confirm(Resources.Common.Confirmation, Resources.Common.DeleteOneRecord, new MessageBoxButtonsConfig
                    {
                        ...
                        {
                            //We are call a direct request metho for deleting a record
                            Handler = String.Format("App.direct.DeleteEH('{0}','{1}')", dayID, employeeid),
                            Text = Resources.Common.Yes
                        },
                        No = new MessageBoxButtonConfig
                        {
        ...
        protected void employeeCalender_RefreshData(object sender, StoreReadDataEventArgs e)
        {
            ...
            ListResponse<EmployeeCal> currencies = _employeeService.ChildGetAll<EmployeeCal>(request);
