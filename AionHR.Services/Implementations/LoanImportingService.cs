   ...
   public class LoanImportingService: ImportingServiceBase<Loan>,IImportaingService
    {
        ...
        //        DateTime effectiveDate = DateTime.Parse(row[8].ToString());
               DateTime effectiveDate = DateTime.ParseExact(row[8].ToString(), "dd-MM-yy", cu);
                ...
                loans.Add(new Loan() { employeeRef = employeeRef, amount = Convert.ToDouble(amount), ltId =ltId, branchId = branch.ToLower()=="null"?null:branch, currencyRef = currRef, date = date, effectiveDate = effectiveDate, loanRef = loanRef, status= Convert.ToInt16(status), purpose=reason ,ldMethod=Convert.ToInt16(ldMe),ldValue = ldVal});
